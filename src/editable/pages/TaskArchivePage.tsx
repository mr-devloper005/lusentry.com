...
const cardBase = 'group block rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-500 hover:-translate-y-1.5 hover:shadow-[0_28px_64px_rgba(49,54,35,0.16)]'
  ...
  return (
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="relative overflow-hidden border-b border-[var(--tk-line)] bg-[linear-gradient(180deg,rgba(98,111,71,0.08)_0%,rgba(255,255,255,0)_100%)]">
          ...
          <div className="pointer-events-none absolute -left-24 top-12 h-52 w-52 rounded-full bg-[var(--slot4-accent-fill)]/20 blur-3xl" />
          <div className="pointer-events-none absolute right-0 top-0 h-56 w-56 rounded-full bg-[var(--tk-accent-soft)] blur-3xl" />
          <div className="relative mx-auto max-w-[var(--editable-container)] px-6 py-20 sm:py-28 lg:px-8">
            ...
            <h1 className="editable-display mt-6 max-w-4xl text-balance text-[2.7rem] font-bold leading-[1.02] tracking-[-0.04em] sm:text-5xl lg:text-[4.4rem]">
              {voice?.headline || `Browse ${label}`}
            </h1>
            <p className="mt-6 max-w-3xl text-lg leading-8 text-[var(--tk-muted)]">{voice?.description || theme.note}</p>
            {voice?.chips?.length ? (
              <div className="mt-8 flex flex-wrap gap-3">
                {voice.chips.map((chip) => (
                  <span key={chip} className="rounded-full border border-[var(--tk-line)] bg-white px-4 py-2 text-xs font-semibold text-[var(--tk-muted)] shadow-[0_12px_24px_rgba(49,54,35,0.07)]">{chip}</span>
                ))}
              </div>
            ) : null}

            <div className="mt-12 flex flex-col gap-4 rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-white/88 p-5 shadow-[0_18px_40px_rgba(49,54,35,0.08)] backdrop-blur-sm sm:flex-row sm:items-center sm:justify-between">
              ...
              <form action={basePath} className="flex items-center gap-2.5">
                <div className="relative">
                  <select
                    ...
                    className="h-11 appearance-none rounded-full border border-[var(--tk-line)] bg-[var(--tk-surface)] pl-4 pr-10 text-sm font-medium text-[var(--tk-text)] outline-none transition focus:border-[var(--slot4-accent-fill)]"
                ...
                <button className="inline-flex h-11 items-center rounded-full bg-[var(--slot4-accent-fill)] px-5 text-sm font-bold text-[#23261d] transition hover:-translate-y-0.5">Apply</button>
              </form>
            </div>
          </div>
        </header>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-6 py-16 sm:py-20 lg:px-8">
          {posts.length ? (
            ...
            <div className="mx-auto max-w-xl rounded-[var(--tk-radius)] border border-dashed border-[var(--tk-line)] bg-[var(--tk-surface)] px-8 py-16 text-center">
              ...
              <h2 className="editable-display mt-5 text-3xl font-bold tracking-[-0.03em]">Nothing here yet</h2>
          ...
          {posts.length ? (
            <nav className="mt-16 flex items-center justify-center gap-3 text-sm">
              {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-full border border-[var(--tk-line)] bg-white px-5 py-2.5 font-semibold transition hover:border-[var(--slot4-accent-fill)]">Previous</Link> : null}
              ...
              {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-full border border-[var(--tk-line)] bg-white px-5 py-2.5 font-semibold transition hover:border-[var(--slot4-accent-fill)]">Next</Link> : null}
            </nav>
          ) : null}
        </section>
      </main>
